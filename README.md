...
`map_kind` works for every kind this extension generates, including the test rule. For
example, to emit `vitest_test` rules (that run Vitest) instead of `jest_test` while keeping
full automatic dependency management:

```starlark
# gazelle:map_kind jest_test vitest_test //build:vitest_test.bzl
```

Generated test targets are then named `vitest_test`, loaded from your `.bzl` file, and still
get their `srcs`, `deps`, `data`, and `config` populated from the test file's imports.
